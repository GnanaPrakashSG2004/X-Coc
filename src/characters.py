...
class Stealth_Archer(Archer):
    def __init__(self, position):
        ...
        self.isInvisible = time.time() - self.initTime <= 10
...
class Healer:
    ...
    def heal_troops(self, pos, King):
        troops = barbarians + archers + stealth_archers + dragons + balloons
        
        for troop in troops:
            if (abs(troop.position[0] - pos[0]) <= self.heal_radius and abs(troop.position[1] - pos[1]) <= self.heal_radius):
                self.heal_target(troop)

        if (abs(King.position[0] - pos[0]) <= self.heal_radius and abs(King.position[1] - pos[1]) <= self.heal_radius):
...
def move_stealth_archers(V,type):
    if(type == 1):
        for stealth_archer in stealth_archers:
            if stealth_archer.isInvisible == True:
                stealth_archer.isInvisible = time.time() - stealth_archer.initTime <= 10
