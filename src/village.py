...
class Village:
    ...
    def generate_map(self):
        ...
        for wall in self.walls_top:                                     # wall is 1x1
            wall_obj = bd.Wall(wall["position"], wall["level"], self)
            self.wall_objs[wall["position"]] = wall_obj
            map[wall["position"][0]][wall["position"][1]] = pt.WALL_TOP
        for wall in self.walls_bottom:
            wall_obj = bd.Wall(wall["position"], wall["level"], self)
            self.wall_objs[wall["position"]] = wall_obj
            map[wall["position"][0]][wall["position"][1]] = pt.WALL_BOTTOM
        for wall in self.walls_left:
            wall_obj = bd.Wall(wall["position"], wall["level"], self)
            self.wall_objs[wall["position"]] = wall_obj
            map[wall["position"][0]][wall["position"][1]] = pt.WALL_LEFT
        for wall in self.walls_right:
            wall_obj = bd.Wall(wall["position"], wall["level"], self)
            self.wall_objs[wall["position"]] = wall_obj
            map[wall["position"][0]][wall["position"][1]] = pt.WALL_RIGHT
        for wall in self.walls_topright:
            wall_obj = bd.Wall(wall["position"], wall["level"], self)
            self.wall_objs[wall["position"]] = wall_obj
            map[wall["position"][0]][wall["position"][1]] = pt.WALL_TOPRIGHT
        for wall in self.walls_topleft:
            wall_obj = bd.Wall(wall["position"], wall["level"], self)
            self.wall_objs[wall["position"]] = wall_obj
            map[wall["position"][0]][wall["position"][1]] = pt.WALL_TOPLEFT
        for wall in self.walls_bottomright:
            wall_obj = bd.Wall(wall["position"], wall["level"], self)
            self.wall_objs[wall["position"]] = wall_obj
            map[wall["position"][0]][wall["position"][1]] = pt.WALL_BOTTOMRIGHT
        for wall in self.walls_bottomleft:
            wall_obj = bd.Wall(wall["position"], wall["level"], self)
            self.wall_objs[wall["position"]] = wall_obj
            map[wall["position"][0]][wall["position"][1]] = pt.WALL_BOTTOMLEFT
        for cannon in self.cannons:                                     # cannon is 2x2
            cannon_obj = bd.Cannon(cannon["position"], cannon["level"], self)
            self.cannon_objs[cannon["position"]] = cannon_obj
            map[cannon["position"][0]][cannon["position"][1]] = pt.CANNON + ":" + \
                str(cannon["position"][0]) + ":" + str(cannon["position"][1])
            map[cannon["position"][0]+1][cannon["position"][1]] = pt.CANNON + \
                ":" + str(cannon["position"][0]) + ":" + str(cannon["position"][1])
            map[cannon["position"][0]][cannon["position"][1]+1] = pt.CANNON + \
                ":" + str(cannon["position"][0]) + ":" + str(cannon["position"][1])
            map[cannon["position"][0]+1][cannon["position"][1]+1] = pt.CANNON + \
                ":" + str(cannon["position"][0]) + ":" + str(cannon["position"][1])
        for wizard_tower in self.wizard_towers:                          # wizard tower is 1X1
            wizard_tower_obj = bd.WizardTower(wizard_tower["position"], wizard_tower["level"], self)
            self.wizard_tower_objs[wizard_tower["position"]] = wizard_tower_obj
            map[wizard_tower["position"][0]][wizard_tower["position"][1]] = pt.WIZARD_TOWER + \
                ":" + str(wizard_tower["position"][0]) + ":" + str(wizard_tower["position"][1])
