...
def printMap(V):
    ...
    for i in range(map.shape[0]):
        for j in range(map.shape[1]):
            if(map[i][j].split(':')[0] == pt.BLANK):
                ...
                map_matrix[2*i][2*j] = Back.BLACK + 'L' + str(V.wall_objs[(i,j)].level)
    ...
    for sArch in stealth_archers:
        ...
        map_matrix[a][b] = archColor(sArch) + Fore.BLACK + 'S '
        map_matrix[a+1][b] = archColor(sArch) + Fore.BLACK + 'CH'
        map_matrix[a][b+1] = archColor(sArch) + Fore.BLACK + 'AR'
        map_matrix[a+1][b+1] = archColor(sArch) + Fore.BLACK + 'ER'
    ...
    for h in healers:
        a= 2*h.position[0]
        b = 2*h.position[1]
        map_matrix[a][b] = hColor(h) + Fore.BLACK + 'HE'
        map_matrix[a+1][b] = hColor(h) + Fore.BLACK + '  '
        map_matrix[a][b+1] = hColor(h) + Fore.BLACK + 'AL'
        map_matrix[a+1][b+1] = hColor(h) + Fore.BLACK + 'ER'
